...
export class MovieRatingAveragePipe implements PipeTransform {
  transform(ratings: MovieRating[]): string {
    const ratingValues = ratings?.map((rating: MovieRating) => rating.value);
    const ratingAverage = ratingValues?.length ? ratingValues.reduce((a, b) => (a + b) / ratingValues.length) : 0;
