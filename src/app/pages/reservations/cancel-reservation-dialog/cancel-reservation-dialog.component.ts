...
export class CancelReservationDialogComponent implements OnInit {
  constructor(@Inject(MAT_DIALOG_DATA) public data: { id: number }, private store: Store<State>) {}

  ngOnInit() {}

  cancelReservation() {
    this.store.dispatch(removeReservationRequest({ payload: { reservationId: this.data.id } }));
  }
}
